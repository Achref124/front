...
export const addFilm =(newFilm)=>async(dispatch) =>{
    ...
    try {
        let result = await axios.post('https://mernproject-sj58.onrender.com/api/film/addFilm', newFilm)
...
export const getFilms =()=>async (dispatch) =>{
    dispatch({type:LOAD_FILM})
    try {
        let result =await axios.get('https://mernproject-sj58.onrender.com/api/film/films')
...
export const deleteFilm =(id) =>async (dispatch)=>{
    dispatch({type:LOAD_FILM})
    try {
       let result=await axios.delete(`https://mernproject-sj58.onrender.com/api/film/deleteFilm/${id}`)
